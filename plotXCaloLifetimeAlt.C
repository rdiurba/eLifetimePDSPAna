...
void plotXCaloLifetimeAlt(std::string runNumber, std::string txtLabel, std::string location, std::string tpc)
{
	ofstream outfile;
	outfile.open(Form("mich1_lifetimeresults_%s_%s_%s_alt.txt",txtLabel.c_str(),location.c_str(),tpc.c_str()),std::ios_base::app);
	...
	std::string set="MC";
	std::string s="MC";
	fileName=Form("Xcalo_mich1_mc_sceOn_%s_%s_alt.root",location.c_str(),tpc.c_str());
	}
        else if (runNumber.find(Form("NoDiff"))!=std::string::npos){
        std::string set="MC No Diff";
        std::string s="MC No Diff";
        fileName=Form("Xcalo_mich1_mc_sceOn_noDiff_%s_%s_alt.root",location.c_str(),tpc.c_str());
        }
	...
	fileName=Form("Xcalo_mich1_r%s_%s_%s_alt.root",runNumber.c_str(),location.c_str(),tpc.c_str());
	}
...
for (int i=1; i<=x_correction_factor->GetNbinsX();i++){
 ...
 rebinHistR->SetBinError(position,x_correction_factor->GetBinError(i));
   ...
   beamRight.push_back(rebinHistR->GetBinContent(position));
    ...
    if (time_BR.size()<3 || time_BL.size()<3) return;
	...
	fit->SetParameter(1,lifetimeEst_BR);
        ...
        if (chi2_dof>1.05){
        for (int bin=0; bin<rebinHistR->GetXaxis()->GetNbins(); bin++){ if (rebinHistRCopy->GetBinContent(bin+1)>0) rebinHistRCopy->SetBinError(bin+1,0.0001+rebinHistRCopy->GetBinError(bin+1));}
       
        }
	if (chi2_dof<0.95){
        for (int bin=0; bin<rebinHistR->GetXaxis()->GetNbins(); bin++){ if (rebinHistRCopy->GetBinContent(bin+1)>0) rebinHistRCopy->SetBinError(bin+1,abs(rebinHistRCopy->GetBinError(bin+1)-0.0001));}
        }
        iter++;
        }
        TLatex tL;
	...
	tL.Draw("SAME");
        c1.Print(Form("fitXCalo%s_BR_%s_%s_alt.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_BR_%s_%s_alt.pdf",s.c_str(),location.c_str(),tpc.c_str()));
	...
	chi2_dof=1000000;
        ...
        if (chi2_dof>1.05){
        for (int bin=0; bin<rebinHistR->GetXaxis()->GetNbins(); bin++){ if (rebinHistLCopy->GetBinContent(bin+1)>0) rebinHistLCopy->SetBinError(bin+1,rebinHistLCopy->GetBinError(bin+1)+0.0001);}
        }
        if (chi2_dof<0.95){
        for (int bin=0; bin<rebinHistL->GetXaxis()->GetNbins(); bin++){ if (rebinHistLCopy->GetBinContent(bin+1)>0) rebinHistLCopy->SetBinError(bin+1,abs(rebinHistLCopy->GetBinError(bin+1)-0.0001));}
        }
        iter++;
        }
	...
	tL.DrawLatex(0.10,0.94,"#bf{DUNE:ProtoDUNE-SP}");
	tL.Draw("SAME");
        c1.Print(Form("fitXCalo%s_BL_%s_%s_alt.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_BL_%s_%s_alt.pdf",s.c_str(),location.c_str(),tpc.c_str()));
	...
	c1.Print(Form("fitXCalo%s_%s_%s_alt.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_%s_%s_alt.pdf",s.c_str(),location.c_str(),tpc.c_str()));
}
