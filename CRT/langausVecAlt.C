...
void langausVecAlt(std::string runNumber) { 
...
//fileName=Form("/dune/data/users/rdiurba/test5759_100.root");
  ...
  //const int tbinsize = 50000;
	  ...
	  double trkzCorr=0,dZOffset=0,dZOffset2=0;
	   ...
	   float eMeas=tot_EfACAData(crtx,crty,trkz->at(hitIndex));
	   ...
	   if (deltaQ>80 && deltaQ1>80 && deltaQ2>80 ) {    hitIndex=hitIndex+1;}
	    ...
	    for (int i =0; i<ntbins; ++i){	
		  ...
		  hSNRVec[i]->SetTitle(Form("%.1f<t_{hit}<%.1f ms",startTime,endTime));
	    ...
	    tL.DrawLatex(0.10,0.94,"#bf{DUNE:ProtoDUNE-SP}");
	     c2->Print(Form("fitLand%s%dAlt.png",s.c_str(),i));    

		   time.push_back((i*tbinsize+tbinsize/2)/1000000.f);
		  etime.push_back(tbinsize/(2000000.f));
		  
		  printf("%d,%f,%f,%f",(i*tbinsize+tbinsize/2),fitsnr->GetParameter(1),fLand->GetParameter(1),fLand->GetMaximumX());
		  printf("\n");
...
int iter=0;
       ...
       if (chi2_dof>1.05){
	    ...
	    double chi_square=fit->GetChisquare();
	    int ndf=fit->GetNDF();
	    ...
	    tL.SetNDC();
	    tL.DrawLatex(0.10,0.94,"#bf{DUNE:ProtoDUNE-SP}");
	  c1->Print(Form("fit%sAlt.png",s.c_str()));
	  c1->Print(Form("fit%sAlt.pdf",s.c_str()));
	  ofstream outfile;
	  outfile.open(Form("crt_lifetimeresults_Nov2018_alt.txt"),std::ios_base::app);
