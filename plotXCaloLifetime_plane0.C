...
void plotXCaloLifetime_plane0(std::string runNumber, std::string txtLabel, std::string location, std::string tpc)
{

	
	ofstream outfile;
	outfile.open(Form("mich1_lifetimeresults_%s_%s_%s_plane0.txt",txtLabel.c_str(),location.c_str(),tpc.c_str()),std::ios_base::app);
	...
	std::string set="MC";
	std::string s="MC";
	fileName=Form("Xcalo_mich1_mc_sceOn_%s_%s.root",location.c_str(),tpc.c_str());
	}
        else if (runNumber.find(Form("NoDiff"))!=std::string::npos){
        std::string set="MC No Diff";
        std::string s="MC No Diff";
        fileName=Form("Xcalo_mich1_mc_sceOn_noDiff_%s_%s.root",location.c_str(),tpc.c_str());
        }
	...
	fileName=Form("Xcalo_mich1_r%s_%s_%s.root",runNumber.c_str(),location.c_str(),tpc.c_str());
	}
...
for (int i=1; i<=x_correction_factor->GetNbinsX();i++){
 ...
 rebinHistR->SetBinError(position,x_correction_factor->GetBinError(i));
   ...
   beamRight.push_back(rebinHistR->GetBinContent(position));
	...
	while (chi2_dof>1.05 || chi2_dof<0.95){
	...
	tL.Draw("SAME");
        c1.Print(Form("fitXCalo%s_BR_%s_%s_plane0.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_BR_%s_%s_plane0.pdf",s.c_str(),location.c_str(),tpc.c_str()));
	...
	tL.DrawLatex(0.10,0.94,"#bf{DUNE:ProtoDUNE-SP}");
	tL.Draw("SAME");
        c1.Print(Form("fitXCalo%s_BL_%s_%s_plane0.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_BL_%s_%s_plane0.pdf",s.c_str(),location.c_str(),tpc.c_str()));
	...
	c1.Print(Form("fitXCalo%s_%s_%s_plane0.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_%s_%s_plane0.pdf",s.c_str(),location.c_str(),tpc.c_str()));
}
