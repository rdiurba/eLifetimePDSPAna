...
void plotXCaloLifetimeAlt_plane1(std::string runNumber, std::string txtLabel, std::string location, std::string tpc)
{

	TCanvas c1=TCanvas();
	ofstream outfile;
	outfile.open(Form("mich1_lifetimeresults_%s_%s_%s_alt_plane1.txt",txtLabel.c_str(),location.c_str(), tpc.c_str()),std::ios_base::app);
	...
	TH1F *x_correction_factor=(TH1F*)my_file1.Get("dqdx_mpv_X_hist_1");
...
for (int i=1; i<=x_correction_factor->GetNbinsX();i++){
 ...
 rebinHistR->SetBinError(position,x_correction_factor->GetBinError(i));
   ...
   beamRight.push_back(rebinHistR->GetBinContent(position));
	...
	tL.Draw("SAME");
        c1.Print(Form("fitXCalo%s_BR_%s_%s_alt_plane1.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_BR_%s_%s_alt_plane1.pdf",s.c_str(),location.c_str(),tpc.c_str()));
	...
	tL.DrawLatex(0.10,0.94,"#bf{DUNE:ProtoDUNE-SP}");
	tL.Draw("SAME");
        c1.Print(Form("fitXCalo%s_BL_%s_%s_alt_plane1.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_BL_%s_%s_alt_plane1.pdf",s.c_str(),location.c_str(),tpc.c_str()));
	...
	c1.Print(Form("fitXCalo%s_%s_%s_alt_plane1.png",s.c_str(),location.c_str(),tpc.c_str()));
	c1.Print(Form("fitXCalo%s_%s_%s_alt_plane1.pdf",s.c_str(),location.c_str(),tpc.c_str()));
}
